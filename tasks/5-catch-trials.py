...
there are also catch trials where led turns on at the wrong time and there is no reward available
"""
...
v.catch_chance = 0.03  # 5% chance of catch trial in each spk change
v.rem_catch_trial = 50  # maximum number of catch trials remaining
...
def trial(event):
    ...
    if event == 'entry':
        ...
        elif random() < v.catch_chance and v.rem_catch_trial > 0:
            goto_state('catch')
        else:
            set_timer('cursor_update', choice(v.sound_bins), False)    
    elif event == 'lick':
...
def catch (event):
    "catch trial where led turns on at the wrong time"
    if event == 'entry':
        hw.light.cue(v.next_led___)
        print('{}, led_direction'.format(v.next_led___))
        pause_timer('trial_timer')
