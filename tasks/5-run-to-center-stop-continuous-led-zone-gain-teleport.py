"""Run-to-center reward-zone task with gain, overshoot, and teleport trials."""
...
# The session begins with a configurable control-only warm-up.  Afterward,
# teleport blocks contain 70% control, 15% forward, and 15% backward trials.
# Triggers are sampled continuously from 35-50% of the visual track.  A
# teleport instantaneously shifts the cursor by 15% without changing the
# measured physical distance.
v.enable_teleport = True
v.teleport_warmup_duration = 15 * minute
v.teleport_control_count = 14
...
v.teleport_trial_type = 'control'
v.teleport_session_phase = 'warmup'
...
def configure_teleport_trial():
    if not v.enable_teleport:
        v.teleport_trial_type = 'control'
        v.teleport_session_phase = 'disabled'
    elif get_current_time() < v.teleport_warmup_duration:
        v.teleport_trial_type = 'control'
        v.teleport_session_phase = 'warmup'
    else:
        v.teleport_session_phase = 'experimental'
        if not v.teleport_trial_queue:
            v.teleport_trial_queue = build_teleport_block()
            v.teleport_block_number += 1
            print('{}, new_teleport_block'.format(get_current_time()))
            print('{}, teleport_block_number'.format(
                v.teleport_block_number,
            ))
            print('{}, teleport_block_order'.format(
                '|'.join(v.teleport_trial_queue),
            ))
    ...
    elif v.teleport_trial_type == 'backward':
        v.teleport_direction = -1
    else:
        v.teleport_trial_type = 'control'
        v.teleport_direction = 0

    if v.teleport_direction:
        v.teleport_trigger_progress = random.uniform(
...
def add_visual_progress(raw_progress):
    """Advance the cursor, applying slower gain once it enters the zone."""
    if raw_progress <= 0:
        return
...
def deliver_teleport():
    ...
    print('{}, teleport_direction'.format(v.teleport_trial_type))
...
def run_start():
    ...
    if min(
        ...
        raise Exception('Teleport block counts cannot be negative')
    if v.teleport_warmup_duration < 0:
        raise Exception('teleport_warmup_duration cannot be negative')
    ...
    print('{}, teleport_warmup_duration_ms'.format(
...
def trial(event):
    if event == 'entry':
        ...
        print('{}, teleport_session_phase'.format(
            v.teleport_session_phase,
        ))
