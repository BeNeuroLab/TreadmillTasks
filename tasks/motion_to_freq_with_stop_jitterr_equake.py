...
def trial(event):
    if event == "entry":
        ...
        # Setup first motion flag
        if v.first_motion_flag is None:
            v.first_motion_flag = True

        # Setup perturbation
        if v.first_motion_flag:
            # Trigger perturbation on a percentage of trials
            v.first_motion_flag = False
            if random.random() < v.sol_thr:
                set_timer("sol_on", v.sol_onset_time, True)
                v.sol_number = random.randrange(12)
                v.sol_off_time = random.choice(v.sol_off_time_list)
                print("{}, Sol_direction".format(v.sol_number))  # Print vars
                print("{}, Sol_duration".format(v.sol_off_time))  # Print vars
