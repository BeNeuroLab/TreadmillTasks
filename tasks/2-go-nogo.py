###############################################################################
# Go / No-Go lick task – cleaned + prints (30-Apr-2025)                       #
###############################################################################
from pyControl.utility import *        # seconds, set_timer, etc.
...
events = ['session_timer', 'lick',
          ...
          'motion']                   # keep if your IMU triggers events
...
v.go_fraction    = 0.70        # Go probability in each block
v.max_run_length = 3           # cap on identical streak length
...
def run_start():
    ...
    make_block()
    print(f'{get_current_time()}, Session Started')
    print(f'{get_current_time()}, CPI {hw.motionSensor.sensor_x.CPI}')
...
def run_end():
    ...
    print('Session Ended')
    print(f'Go   : {v.correct_go_trials}/{v.total_go_trials}   ({go_p:.2f}%)')
    print(f'NoGo : {v.correct_nogo_trials}/{v.total_nogo_trials} ({nogo_p:.2f}%)')
    print(f'Overall : {v.correct_go_trials+v.correct_nogo_trials}/{total} ({overall_p:.2f}%)')
...
def stimulus_on(event):
    if event == 'entry':
        ...
        print(f'{get_current_time()}, frequency {v.current_stim_freq}')
...
def response_window(event):
    if event == 'entry':
        ...
        if v.current_stim_freq == v.nogo_stim_freq:
            ...
            print(f'Correct NoGo trials: {v.correct_nogo_trials}/{v.total_nogo_trials}')
            v.last_trial_correct = True
        else:
...
def reward(event):
    if event == 'entry':
        ...
        v.last_trial_correct = True
        print(f'reward number {v.correct_go_trials}/{v.total_go_trials}')
        reset_timer('timeout_timer', v.target_duration)
...
def punish_timeout(event):
    if event == 'entry':
        v.last_trial_correct = False
        print(f'Punishment Timeout Started ({v.punish_timeout_duration/second}s)')
