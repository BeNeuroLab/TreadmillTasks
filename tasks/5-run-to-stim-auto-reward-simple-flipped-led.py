"""Run to target with center-to-side LED feedback, target tone, then auto reward."""
...
def led_percent_for_step(step):
    """Map feedback step to LED percent, center at start and side at target."""
    progress = max(0.0, min(1.0, step / v.feedback_steps))
    percent = int(100 - ((100 - v.side_led_percent) * progress))
...
def apply_feedback_step(step, play_tone=False):
    ...
    if play_tone or v.play_progress_tones:
...
def update_feedback_from_distance(force=False):
    ...
    if force:
        apply_feedback_step(v.feedback_steps, play_tone=True)
    elif (
