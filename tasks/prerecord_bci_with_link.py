...
def intertrial(event):
    if event == 'entry':
        hw.bci_link.send_int_to_bci(0) # Indicate start of intertrial to BCI
        hw.speaker.off()
        ...
        hw.bci_link.send_int_to_bci(3) # Indicate lick to BCI
...
def trial(event):
    if event == 'entry':
        hw.bci_link.send_int_to_bci(1) # Indicate start of trial to BCI
...
def reward(event):
    if event == 'entry':
        ...
        hw.bci_link.send_int_to_bci(3) # Indicate lick to BCI
        hw.bci_link.send_int_to_bci(2) # Indicate reward given to BCI
        v.reward_number += 1
        hw.reward.release()
        hw.speaker.off()
