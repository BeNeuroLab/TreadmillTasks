"""Run-to-center task with slower reward-zone gain and a saturating LED."""
...
def run_start():
    ...
    if v.stop_to_reward_timeout <= 0:
        raise Exception('stop_to_reward_timeout must be positive')
...
def target_wait(event):
    if event == 'entry':
        ...
        record_failure('reward_zone_timeout')
