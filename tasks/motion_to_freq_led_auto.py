...
events = [
    ...
    'stop_button',
    'check_stop_timer'    # New event for auto-reward on stop
]
...
def reward(event):
    if event == 'entry':
        ...
        # Check if already stopped enough
        time_since_last_motion = get_current_time() - v.last_motion_time
        if time_since_last_motion >= v.no_motion_before_reward:
             # Already met condition
             set_timer('check_stop_timer', 1, True) # trigger immediately
        else:
             # Need to wait remaining time
             remaining_time = v.no_motion_before_reward - time_since_last_motion
             set_timer('check_stop_timer', remaining_time, True)

    elif event == 'exit':
        disarm_timer('reward_timer')
        disarm_timer('check_stop_timer')

    elif event == 'motion':
        v.last_motion_time = get_current_time()
        # Reset stop requirement
        set_timer('check_stop_timer', v.no_motion_before_reward, True)
        
    elif event == 'check_stop_timer':
        # Condition met!
        v.reward_number += 1
        hw.reward.release()
        hw.speaker.off()
