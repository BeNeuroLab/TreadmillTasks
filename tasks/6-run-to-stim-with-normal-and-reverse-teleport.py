...
v.normal_teleport_prob = 0.2    # Probability of forward teleport to goal per block
v.reverse_teleport_prob = 0.2   # Probability of reverse teleport to start per block
v.teleport_idxs = [3, 5, 7]     # Step-change indices that can trigger teleport
v.teleport_block_size = 10      # Trials per shuffled teleport-type block
v.teleport_trial_type = 'none'  # 'none', 'normal', or 'reverse'
v.teleport_trigger_index = 0    # Which step triggers the teleport
v.teleport_done = False         # Prevents repeated teleport in a single trial
...
def build_trial_type_block():
    block_size = int(v.teleport_block_size)
    if block_size < 1:
        print('Warning: teleport_block_size < 1, using 1')
        block_size = 1

    num_normal = max(0, int(block_size * v.normal_teleport_prob))
    num_reverse = max(0, int(block_size * v.reverse_teleport_prob))

    if num_normal > block_size:
        print('Warning: normal_teleport_prob exceeds block capacity; clamping')
        num_normal = block_size
    if num_normal + num_reverse > block_size:
        print('Warning: teleport probabilities exceed block capacity; clamping reverse teleports')
        num_reverse = block_size - num_normal

    num_none = block_size - num_normal - num_reverse
    block = (
        ['normal'] * num_normal +
        ['reverse'] * num_reverse +
        ['none'] * num_none
...
def choose_teleport_trial_type():
    if not v.trial_type_sequence:
        v.trial_type_sequence = build_trial_type_block()
        print('New trial block generated: {}'.format(v.trial_type_sequence))

    v.teleport_trial_type = v.trial_type_sequence.pop(0)
    v.teleport_done = False
    v.teleport_trigger_index = 0

    if v.teleport_trial_type == 'none':
        print('No Teleport Trial')
        return

    if not v.teleport_idxs:
        print('Warning: empty teleport_idxs; converting to no teleport trial')
        v.teleport_trial_type = 'none'
        return

    v.teleport_trigger_index = random.choice(v.teleport_idxs)
    print('{} Teleport Trial! Trigger on update #{}'.format(
        v.teleport_trial_type,
        v.teleport_trigger_index
    ))

def apply_teleport_if_due():
    if v.teleport_done:
        return
    if v.teleport_trial_type == 'none':
        return
    ...
    if v.teleport_trial_type == 'normal':
        v.current_distance = v.goal_distance
        print('{}, normal_teleport_to_goal'.format(get_current_time()))
    elif v.teleport_trial_type == 'reverse':
        v.current_distance = 0
        set_start_feedback()
        print('{}, reverse_teleport_to_start'.format(get_current_time()))

def update_feedback_from_distance():
    """Update speaker and LED feedback based on current distance."""
    apply_teleport_if_due()
...
def run_start():
    ...
    print('{}, normal_teleport_prob'.format(v.normal_teleport_prob))
    print('{}, reverse_teleport_prob'.format(v.reverse_teleport_prob))
