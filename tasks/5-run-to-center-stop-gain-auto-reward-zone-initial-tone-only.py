"""Run to a near-center reward LED zone with initial tone only."""
...
# LED states.  Reward is available from the first entry at or above 92.
v.led_positions_csv = '35,42,49,56,63,70,77,84,92,100'
v.reward_zone_start_led_percent = 92
v.led_positions = []
...
def reward_zone_start_index():
    for i in range(v.num_led_positions):
        if v.led_positions[i] >= v.reward_zone_start_led_percent:
            return i
    return target_led_index()


def in_reward_zone():
    return v.current_led_index >= reward_zone_start_index()
...
def run_start():
    ...
    print('{}, reward_zone_start_index'.format(reward_zone_start_index()))
