"""Compare LED-update blocks with a flashing reward-zone target."""
...
v.target_blink_period = 250 * ms
v.target_blink_on = False
...
def start_target_blinking():
    v.target_blink_on = True
    show_current_led()
    set_timer('target_blink_timer', v.target_blink_period)
    print('{}, target_blink_start percent={} period={}'.format(
        get_current_time(),
        v.current_led_percent,
        v.target_blink_period,
    ))


def toggle_target_blink():
    if v.target_blink_on:
        set_led_baseline()
        v.target_blink_on = False
    else:
        show_current_led()
...
def stop_target_blinking():
    disarm_timer('target_blink_timer')
    v.target_blink_on = False
...
def run_start():
    ...
    print('{}, target_blink_period'.format(v.target_blink_period))
...
def target_wait(event):
    if event == 'entry':
        ...
        v.last_motion_time = get_current_time()
        update_cues_from_progress()
        if not v.target_blink_on:
            set_led_baseline()

    elif event == 'target_blink_timer':
        toggle_target_blink()
        set_timer('target_blink_timer', v.target_blink_period)
