"""task with 3 LEDs but short cursor match duration
+ a timeout
target changes only after a reward or a timeout, not after a penalty
target is always on during the speaker sweep. 
"""
...
# v.spks___ = [2, 3, 4]
v.spk_freqs___ = [15000, 15000]
v.leds___ = [2, 4]
v.next_led___ = v.leds___[-1]
v.next_spk___ = v.spk_freqs___[-1]
...
def reward (event):
    ...
    if event == 'entry':
        ...
        if v.next_led___ == v.leds___[0]:
            v.next_spk___ = v.spk_freqs___[0]
        else:
            v.next_spk___ = v.spk_freqs___[1]
